...
class Trainer(object):
    ...
    def __init__(self, args, task, model, criterion, dummy_batch=None, oom_batch=None):
        ...
        if args.fp16:
            ...
            #self._model.decoder.pic_head.init_float32_params()
        if self.cuda:
    ...
    def model(self):
        if self._wrapped_model is None:
            ...
            #self._wrapped_model.decoder.pic_head.init_buffers()
            ...
            logging_outputs, sample_sizes, ooms, prev_norms = \
                zip(*distributed_utils.all_gather_list(
                    [logging_outputs, sample_sizes, ooms,
                        self._prev_grad_norm.item() if isinstance(self._prev_grad_norm, torch.Tensor) else self._prev_grad_norm],
                ))
